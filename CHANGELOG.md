# Changelog / Histórico

## 0.3.0 — 2026-09-10 (experimental)

- Length-based segment editing, optional automatic packing and dynamic equal distribution.
- Rainbow, Chase, Breathe and Color loop effects, with bounded updates and shared region budget.
- Effect tasks stop on external control, connection failure and unload. Existing IDs/settings remain compatible.
- Edição por quantidade de blocos, organização automática opcional e distribuição dinâmica.
- Efeitos com frequência limitada e divisão do espaço disponível no comando.
- Interrupção em controle externo, falhas e recarga; compatibilidade com IDs e configurações existentes.
- Version 0.2.1 validated in HA; new 0.3.0 behavior awaits hardware validation.

## 0.2.1 — 2026-09-10 (experimental)

- Prepare public repository, MIT license, bilingual documentation, HACS metadata
  and CI validation. Require the user's own IP and installed block count at setup.
- Preparação para publicação com licença MIT, documentação bilíngue, metadados HACS
  e CI. Configuração exige o IP e a quantidade real de blocos do usuário.

## 0.2.0 — 2026-09-10 (local prototype / protótipo local)

- Add RGBWW entities and visually verified warm/cool channel mapping.
- Entidades RGBWW e mapeamento dos brancos quente/frio confirmado visualmente.

## 0.1.0 — 2026-09-10 (local prototype / protótipo local)

- Initial local UDP client, named RGB segments, configuration flow and tests.
- Cliente UDP local, segmentos RGB nomeados, configuração e testes iniciais.
